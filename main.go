...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	if t.Duration == 0 {
		fmt.Println("t.Duration = 0. Ошибка, делить на ноль нельзя")
		return 0
	}
	durationInHour := t.Duration.Hours()
	return t.distance() / durationInHour
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	durationInHour := r.Duration.Hours()
	return ((float64(CaloriesMeanSpeedMultiplier)*r.meanSpeed() + CaloriesMeanSpeedShift) * float64(r.Weight) / MInKm * durationInHour * MinInHours)
}
...
const (
	...
	SmInM                         = 100   // коэффициент для перевода роста из см в м
)
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	durationInHour := w.Duration.Hours()
	// Переводим значение роста из см в м
	heightInM := w.Height / SmInM
	// Переводим значение средней скорости из Км/ч в М/с
	speedInMsec := w.meanSpeed() * KmHInMsec
	return ((float64(CaloriesWeightMultiplier)*float64(w.Weight) + (math.Pow(speedInMsec, 2)/heightInM)*CaloriesSpeedHeightMultiplier*float64(w.Weight)) * durationInHour * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	durationInHour := s.Duration.Hours()
	return float64(s.LengthPool*s.CountPool) / MInKm / durationInHour
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	durationInHour := s.Duration.Hours()
	return (s.meanSpeed() + float64(SwimmingCaloriesMeanSpeedShift)) * SwimmingCaloriesWeightMultiplier * float64(s.Weight) * durationInHour
}
